...
DATABASE_db1_URL = "mysql://admin:Seigakushakorea0308(!@127.0.0.1/TripTaleDB"
...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "/home/Seigakushakorea/kjh8/config/인증 키"
...
async def tagchoice(data: tagChoice, token = Depends(oauth2_scheme)):
    ...
    try:
        ...
        if user_data['speechText']:
            TagRecommend.append(f"{user_data['speechText']}で")

        if user_data['time']:
            TagRecommend.append(f"{user_data['time']}間旅行できる")

        if user_data['location']:
            TagRecommend.append(f"{user_data['location']}場所をおすすめして。")
        ...
        if addition_TagRecommend:
            ...
            TagRecommend.append(f"やってみたいのは{addtion_text}だよ。 これで旅行ル―トを作ってくれ。")   
...
async def get_tts(data: ttsRequest):
    ...
    voice = texttospeech.VoiceSelectionParams(
        language_code="ja-JP",  # 일본어로 설정
        name="ja-JP-Wavenet-A"  # 일본어 Wavenet 음성 사용
    )
...
async def extract_place_keywords(token=Depends(oauth2_scheme)):
    try:
        ...
        response = await openai.ChatCompletion.acreate(
            ...
            messages=[
                {"role": "system", "content": "You are a helpful assistant."},
                {"role": "user", "content": f"次のテキストから固有名詞の場所に関連するキーワードを抽出してください: {text}. キーワードをコンマで区切られたリスト形式で返してください。"}
            ],
            max_tokens=500,
            temperature=0.3
        )
