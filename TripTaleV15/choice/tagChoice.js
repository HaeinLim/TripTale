...
function generateSubOptions() {
  ...
  if (gridContainer) {
    ...
    console.error("gridContainerが見つかりません。");
  }
}
...
document.getElementById('nextBtn').addEventListener('click', function() {
  ...
  const food = selectedOptions.includes('Food') ? '食' : null;
  const dessert = selectedOptions.includes('Dessert') ? 'デザート': null;
  const shopping = selectedOptions.includes('Shopping') ? '買い物': null;
  const history  = selectedOptions.includes('History') ? '歴史': null;
  const experience = selectedOptions.includes('Experience') ? '体験': null;
  const nature = selectedOptions.includes('Nature') ? '自然': null;
